...
from convolutional_lstm_multiple_places import ConvolutionalLSTMMultiplePlaces
...
def process(conv_filters, place_group_name, places_group, lstm_state_size, num_lags_back, mini_batch_size, num_epochs):
    nn = ConvolutionalLSTMMultiplePlaces(
    ...
    open(('./res/individual_%s_conv_lstm_multiple_places_at_once.txt' % (place_group_name,)), 'w')\
    ...
    for group_name, group_places in [('middle', selected_middle_of_roads), ('junction', selected_junctions)]:
        open(('./%s_%s_conv_lstm_all_places_at_once.txt' % (place_group_name, group_name,)), 'w')\
...
if __name__ == '__main__':
    ...
    #     'all_middle_and_junction_randomly_ordered', all_middle_and_junction,
