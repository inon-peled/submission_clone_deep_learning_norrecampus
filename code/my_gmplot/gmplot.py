...
from .map_style import map_style
...
class GoogleMapPlotter(object):
    ...
    def marker(self, lat, lng, color='#FF0000', c=None, label=None):
        if c:
            color = c
        color = self.color_dict.get(color, color)
        color = self.html_color_codes.get(color, color)
        self.points.append((lat, lng, color[1:], label))
    ...
    def weighted_heatmap(self, weighted_lats_lngs,
                         threshold=10, radius=10, gradient=None, opacity=0.6, dissipating=True):
        """
        :param weighted_lats_lngs: dict of tuple (lat, lng) to float weight
        :param threshold:
        :param radius: The hardest param. Example (string):
        :return:
        """
        settings = self._process_heatmap_kwargs({
            'threshold': threshold,
            'radius': radius,
            'gradient': gradient,
            'opacity': opacity,
        ...
        self.weighted_heatmap_points.append((weighted_lats_lngs, settings))
    ...
    def write_point(self, f, lat, lon, color, label):
        ...
        if label:
            f.write('\t\tlabel: {text: "%s", color: "white", fontWeight: "bold", fontFamily: "monospace", fontSize: "22px"},\n' % (label,))
        f.write('\t\ticon: img,\n')
    ...
    def write_weighted_heatmap(self, f):
        for weighted_heatmap_points, settings_string in self.weighted_heatmap_points:
            f.write('var weighted_heatmap_data = [\n')
            for lat_lng, weight in weighted_heatmap_points.items():
                # {location: new google.maps.LatLng(37.782, -122.447), weight: 0.5},
                f.write('{location: new google.maps.LatLng(%f, %f), weight: %f},' % (lat_lng[0], lat_lng[1], weight))
            f.write('];\n')
            f.write('''
            var heatmap = new google.maps.visualization.HeatmapLayer({
                data: weighted_heatmap_data
            });
            heatmap.setMap(map);
            ''')
            f.write(settings_string)
...
if __name__ == "__main__":
    ...
    mymap.weighted_heatmap({(37.429, -122.150288): 3.1, (37.431257, -122.149794): 1.2})
